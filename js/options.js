$(document).ready(function(){
    ...
    jQuery.fn.anim_progressbar = function (aOptions) {
        ...
        // def options
        var aDefOpts = {
            start: new Date().setTime(new Date(2012, 06, 03).getTime()), // now
            finish: new Date().setTime(new Date(2013, 07, 28).getTime()-12*iHms), // now + 60 sec
            interval: 100,
            type : ''
        }
                    ...
                    function(){
                        ...
                        if(aOpts.type == 'iSec'){
                            ...
                            if(last_value != last_value_iSec)
                                $(Pb).find('.drop').drop(500 - iPerc * 3.75)
    ...
    var iEnd = new Date().setTime(new Date(2012, 06, 03).getTime()); // now plus 15 secs
    ...
    $("#subscribe").submit(function(event) {
        ...
        if ($.trim(sEmail).length == 0) {
            $('#contactresult').html('<p class="error one">Please enter a valid email address</p>');
            event.preventDefault();//stops script execution
            return false;
        }
        //if valid email send info to php
        if (validateEmail(sEmail)) {
            $.post("php/subscribes.php", $("#subscribe").serialize(),function(result){
                $('#contactresult').html(result);
            }); 
        } else {
            $('#contactresult').html('<p class="bad">Invalid Email Address.Please try again</p>');
            event.preventDefault();
            return false;
        }
    });
    ...
    setInterval(function(){
        $('.wt_twitter_post').eq(i).fadeOut('slow', function() {
            i++;
